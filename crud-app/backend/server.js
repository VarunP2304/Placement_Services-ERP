...
app.post('/api/students', upload.fields([
  ...
  const offer_letter = req.files?.['offer_letter'] ? req.files['offer_letter'][0].filename : null;
  const internship_letter = req.files?.['internship_letter'] ? req.files['internship_letter'][0].filename : null;
...
app.put('/api/students/:usn', upload.fields([
  { name: 'offer_letter', maxCount: 1 },
  { name: 'internship_letter', maxCount: 1 }
]), (req, res) => {
...
app.put('/api/companies/:id', (req, res) => {
  ...
  db.query(query, [name, job_role, description, package, companyId], (err, result) => {
    if (err) {
